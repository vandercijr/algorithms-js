...
const typeFactor = (type) => (type === "put" ? -1 : 1);

export const blacknscholes = {
  //d1 term from EDP
  d1Term: (stock_price, strike, volatility, interest_rate, expiration_days) =>
    (Math.log(stock_price / strike) +
      (interest_rate + Math.pow(volatility, 2) * 0.5) * expiration_days) /
    (volatility * Math.sqrt(expiration_days)),
  //d2 term from EDP
  d2Term: (d1term, volatility, expiration_days) =>
    d1term - volatility * Math.sqrt(expiration_days),
  ...
  price: (
    ...
    return (
      tf * (stock_price * nd1) -
      tf * (strike * eulerRateTime(interest_rate, expiration_days) * nd2)
    );
  },
  //delta greek measure
  delta: (d1term, type) => {
  ...
  rho: (strike, interest_rate, expiration_days, d2term, type) => {
    ...
    return (
      ...
      strike *
      expiration_days *
      eulerRateTime(interest_rate, expiration_days) *
      gauss.standardNormalDistribution(d2term * tf) *
      0.01
    );
  },
  //gamma greek measure
  gamma: (stock_price, volatility, expiration_days, d1term) =>
    ...
    gauss.spdf(d1term) /
    (stock_price * volatility * Math.sqrt(expiration_days)),
  //vega greek measure
  vega: (stock_price, expiration_days, d1term) =>
    // from https://financetrain.com/option-greeks-rho/
    // formula is vega = S . N'(d1) /  SQRT(t)
    stock_price * gauss.spdf(d1term) * Math.sqrt(expiration_days) * 0.01,
  //theta greek measure
  theta: (
    stock_price,
    strike,
    ...
    return (
      ((-1 * stock_price * gauss.spdf(d1term) * volatility) /
        (2 * Math.sqrt(expiration_days)) -
        tf *
          interest_rate *
          strike *
          eulerRateTime(interest_rate, expiration_days) *
